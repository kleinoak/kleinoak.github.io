...
**`site.scheduleUpdated` was retired on 2026-09-06.** It carried the date a person last reconciled the schedule and results against Rank One, and it was rendered in one place: a line inside the Rank One callout on the Schedule page. When the automated sync stamp moved into that callout the line came out, which left a field an editor could still fill in and nobody would ever see — so the field went too, from the schema, from `site.ts` and from `content/site.json`. **Nothing on the site now says when a *person* last checked the start times**, only when the *scraper* last ran; the two are not the same claim, and the difference is recorded under [Known Limitations](#known-limitations). The last stored value was `2026-08-25`, and git has it.
...
│   │   ├── home/HeroCarousel.tsx   # rotating hero; CSS-stacked, fixed height
...
│   │   ├── schedule/SyncStatus.tsx      # "last checked" band inside the callout
...
`HeroCarousel` stacks **every slide in one CSS grid cell**, so the browser sizes the box to the tallest of them and the height cannot change when the banner does. Only the active slide is visible; the rest are `visibility: hidden` plus `inert`, which reserves their space while keeping their links out of the tab order and the accessibility tree — tabbing into content nobody can see is the standard carousel accessibility bug. The first slide is server-rendered, so the program hero is the first paint and the only slide that appears with JavaScript disabled, and because the height is pure CSS it is already correct in that HTML.

It used to measure instead — hold the tallest height seen so far in state, and drop that floor on `resize` so a desktop measurement did not leave a black band after rotating to a phone. On iOS that is a trap: scrolling collapses Safari's URL bar, which fires `resize`, which dropped the floor and snapped the hero from 677px to 318px — yanking the page up 359px under the reader's thumb, mid-scroll, with no banner change involved. The layout engine already knows the answer; asking JavaScript for it is what created the bug.
...
**The page now says one of the two, at the top.** `SyncStatus` is the bottom band of the Rank One callout — "Last checked Sun, Sep 6 at 12:32 PM Central", the date the data last actually moved, and the fixture count. It covers the automated half only. The human half — "start times and tournament records are reconciled by hand, last done on …" — used to sit in the same callout as a separate line and was removed on 2026-09-06, so the distinction between *a parser read four calendars* and *a person decided which fixture belongs to which row* is no longer drawn anywhere a reader can see.

**The stamp is only as fresh as the last *publish*, and for weeks the mirror was fetching data that never got published.** `rankone.yml` runs in the engineering repository and its `publish` job force-pushes to production, which is what makes the page's stamp move. A copy of the same workflow was also reaching the mirror and running there, where its commits could not trigger a build — see [Known Limitations](#known-limitations). Fixed on 2026-09-07; the engineering copy is the only one that runs now.

**The stamp has been in three places, and this is the reasoning each time.** It began above the callout, moved to the foot of the page on the argument that provenance is something a reader asks *after* the schedule, and came back up on 2026-09-06 — this time *inside* the callout rather than competing with it. The foot was worse than it sounded: it stranded the answer a thousand pixels below the question, past the fixtures and Program Dates, when the page's own hero says "times can change late in the week". One panel now carries what the live source is, the way to it, and when it was last read.

**`SyncStatus` supplies no chrome of its own.** No background, no rounding, no side border — just a hairline above it and the callout's padding, so it reads as the panel's bottom band rather than a card dropped inside one. That makes it **not a free-standing component**: rendering it anywhere else means giving it a container there.
...
| `standard` | 96px | 128px tall, ordinary card border | two / three / four across |
...
**A logo with a `url` is a link, and the whole plate is the target.** `content/sponsor-logos.json` carries an optional `url` per business; when it is set the plate renders as an `<a target="_blank" rel="noopener noreferrer">` and the artwork lifts 3% on hover. The link's accessible name is the alt text plus an `sr-only` "(opens in a new tab)" — the same phrasing the sponsorship-form link on the same page uses. `rel` matters more here than elsewhere on the site: these are other people's domains, and without `noopener` the opened tab gets a handle on this one through `window.opener`.

The hover lift only exists when an ancestor carries `group`, which only the linked branch adds — so **an unlinked plate is completely still** and never suggests it can be clicked. A sponsor with no `url` is a plain `<div>`, not a dead link; a business with no website, or one that has not asked to be linked, is normal.

A sponsor with no artwork at all renders as its business name, at a font size that scales with the tier — so a platinum sponsor whose logo has not arrived yet still gets the prominence they paid for. That branch has no link, because the `url` lives on the logo entry.

**The standard cap is 96px because that is what the plate takes.** It was 80px, which left the tallest artwork in the tier rendering at barely half the plate's usable width while the wide wordmarks beside it filled every pixel. 96px is a 128px plate with 16px of air above and below. It cannot go higher without either touching the border or lifting the black tier to within a whisker of gold, and that ceiling has a consequence worth knowing before anyone tries: see [Known Limitations](#known-limitations) on the State Farm card.
...
**`flyer.focus` moves that band when the middle is the wrong place to cut.** The card shows about a third of the poster's height, and centring it is right only when the middle is the identifying part. The Middle School Invitational poster's title is its identity, and centred the band sliced the word INVITATIONAL in half — which reads as a rendering fault, not a crop. `focus` is a CSS `object-position` stored in the content (`"center 17%"`), applied as an **inline style** rather than a Tailwind class, because the value comes from JSON and Tailwind only compiles class names it can see in the source. Pick it by measuring: find the poster's bands of ink, then choose a percentage that puts both cuts in a gap rather than through a line of type.
...
- **The schedule is still a hand-made copy in the ways that matter.** A job now re-reads all four Rank One calendars three times a day (see [Keeping the schedule in sync](#keeping-the-schedule-in-sync)), which covers venues, maps, per-game notes and newly posted scores. It does **not** cover the parts that need judgement: start times in `matches.json` are still transcribed by hand, nothing warns when Rank One changes one, tournament records are still totted up by a person, and nothing notices a fixture that was played and never scored. **The page no longer says so.** Until 2026-09-06 two dates sat on it and the pair drew the line — the scraper's stamp and a hand-reconciliation date — but the hand date was removed when the sync stamp moved into the Rank One callout, and its content field was retired with it. A reader now sees "last checked 12:32 PM" and has no way to tell that the *start times* behind it were last verified by a person on 2026-08-25. Restoring the disclosure is a line in `SyncStatus` plus putting the field back; the git history has both.
...
- **`deploy-prod.sh` was macOS-only until CI ran it.** `mktemp -t ko-prod-index` is a BSD spelling: it treats the argument as a prefix and invents the suffix, while GNU mktemp rejects a template with no `XXXXXX`. It worked for every hand deploy from a Mac and failed the first time the publish job ran it on Ubuntu. Fixed, but it is the class of bug to expect from a shell script that only ever ran on one operating system, and nothing else in the repository is exercised on two.
- **A pipeline hid that failure and reported success.** `sh scripts/deploy-prod.sh | tee` takes `tee`'s exit status, and GitHub's default `bash -e` shell does not set `pipefail` — so the deploy died, printed half its output, and the job went green while production was never touched. The step now sets `pipefail` and greps the log for the script's success line. **Any `| tee` in a CI step has this bug unless pipefail is on.**
- **Production ran a second copy of the Rank One scraper for weeks, and every run was green and pointless.** `deploy-prod.sh` stripped only `*.md`, so each deploy shipped `.github/workflows/rankone.yml` to the mirror, where GitHub duly ran it on the same three-times-a-day cron. The workflow's own header had predicted half of this — a commit inside the mirror is force-pushed away by the next deploy — but not the half that actually bit: **the mirror's commits are made with `GITHUB_TOKEN`, and a `GITHUB_TOKEN` push triggers no other workflow**, so `deploy.yml` never fired and the fetched data never reached a build. Three prod-local syncs on 2026-09-06 were each followed by a build that came from *engineering's* publish job; the 04:12 UTC run on 2026-09-07 fetched a real fixture change and was followed by nothing at all, which is how it was noticed — a reader saw a sync in the Actions tab and a stale timestamp on the page. Fixed in two places: the deploy script strips the file (and asserts it), and the sync job carries `if: github.repository != 'kleinoak/kleinoak.github.io'` so an older deployed tree still refuses to run it. **The general rule: a workflow that writes to the repository it runs in must never be shipped to a mirror.** `pull-request.yml` is still shipped and is harmless — nobody opens PRs there — but it is the same class of thing.
- **Engineering's scheduled runs land hours late, and that is normal GitHub behaviour.** The `0 4`, `0 11`, `0 15` crons in `rankone.yml` fired at 08:15, 13:50 and 17:31 UTC on 2026-09-06 — between two and a half and four hours after their nominal time. GitHub queues scheduled workflows at low priority and makes no timeliness guarantee. Production's copy of the identical cron ran within twelve minutes every time, which is why the mirror kept winning the race to fetch. Nothing is wrong and nothing can be done about it, but "the sync did not run on time" is not evidence of a fault, and the page's own "last checked" stamp is the honest reading of it.
...
- **One sponsor's artwork is a business card, and its address block cannot be made legible.** Blaine Scelfo's file carries a photograph, the State Farm mark, a name, a street address and a phone number. Raising the standard cap from 80px to 96px on 2026-09-07 took it from 126×80 to 151×96, which is enough for the State Farm mark and "Blaine Scelfo" and **not** enough for the three red contact lines: they are ~22px of a 381px-tall source, so at 96px they render about 5.5px tall. Making them legible needs roughly 156px of render height in a 128px plate — it is arithmetic, not styling, and no cap fixes it. The artwork cannot be cropped either: the red wedge and the portrait bleed to both the top and bottom edges, and only the white text panel has any margin. Two things that would work, neither of them code: ask the sponsor for a plain mark with no address block (it would render near full width), or accept that **the plate now links to their site**, which is where a reader gets the address anyway.
