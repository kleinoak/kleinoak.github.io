...
**Varsity results come from Rank One, and only varsity.** A match may carry a `result` — `"W 3–0"`, `"L 0–2"`, or a record like `"5–1"` for a tournament played across several matches — transcribed from the program's Rank One varsity calendar. Rank One publishes results per team and only the varsity feed is sourced, so the Result column appears **only** when varsity times are on screen (the All view and the Varsity filter) and only in a section that has at least one result. Showing an always-empty column to a JV parent would imply their results were missing rather than not tracked. **A blank result means no result has been posted — never a loss and never a cancellation** — so the cell renders as a dash with "No result posted" for screen readers, and nothing infers an outcome from absence.
